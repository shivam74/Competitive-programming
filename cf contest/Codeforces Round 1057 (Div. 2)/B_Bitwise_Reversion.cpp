...
void solve(){
    ll x,y,z;
    cin>>x>>y>>z;
    if(x==y && y==z && z==x){cout<<"YES"<<endl; return;}
    // if(x==y && z>0){
    ...
    // ans&=y;
    // ans&=z;
    ll a=__builtin_popcountll(x);
    ll b=__builtin_popcountll(y);
    ll c=__builtin_popcountll(z);
    // // //cout<<ans<<endl;
    // //cout<<a<<" "<<b<<" "<<c<<endl;
    if(a==b && b==c){
        cout<<"YES"<<endl;
    }
    else if((a<c && b<c) || (c<a && b<a) || (a<b && c<b))  {cout<<"YES"<<endl;}
    //if((x&y)>1 || (y&z)>1 || (z&x)>1)cout<<"YES"<<endl;
    else{
        cout<<"NO"<<endl;
    }
}
