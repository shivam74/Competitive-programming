...
void dfs(vector<vector<ll>> &g,vector<ll> &vis,ll node,ll par,vector<pair<ll,ll>> &reject){
    vis[node]=1;
    for(auto &adj:g[node]){
        if(adj==par)continue;
        else if(!vis[adj]){
            dfs(g,vis,adj,node,reject);
        }
        else{
            reject.push_back({node,adj});
        }
    }
}
...
void solve(){
    ...
    vector<ll> vis(2*n+1);
    vector<pair<ll,ll>> reject;
    for(ll i=1;i<=2*n;i++){
        if(!vis[i]){
            dfs(g,vis,i,-1,reject);
        }
    }
    // for(auto &p:reject){
